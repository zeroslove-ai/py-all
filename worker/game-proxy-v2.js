...
function buildNpcLocationRules() {
  return `\n\n[REGISTERED NPC LOCATION RULE]\n3병동은 heroine1, heroine2, heroine3, heroine4, heroine9, heroine10만, 6병동은 heroine5·heroine6만, 의사 중심 장면은 heroine7·heroine8만 상호작용한다. 등록되지 않은 직원·환자·보호자는 이름 없는 배경 묘사만 가능하며 먼저 말하거나 선택지 대상이 될 수 없다. 새 고유 NPC 이름을 만들지 말고 외형만 보고 heroine ID를 추측하지 마라.`;
}
...
function normalizeRegisteredNpcExtract(extract = {}, characters = {}, lastCharacterId = null) {
  const normalized = isPlainObject(extract) ? { ...extract } : {};
  ...
  const names = new Set([...ids].map(id => characters?.[id]?.name || characters?.[id]?.['이름']).filter(Boolean));
  normalized.dialogue_lines = Array.isArray(normalized.dialogue_lines)
    ? normalized.dialogue_lines.filter(line => isPlainObject(line) && names.has(line.speaker))
    : [];
