...
**사용자 지시**: 플레이어 정보 필수 입력 + 프리셋 추출 버그 수정

**주요 변경**:
- `c_llm` (플레이 LLM) 프롬프트 [플레이어 정보 확인] 수정
  - "합리적인 기본값으로 진행" → **완전 제거**
  - 이름 + 직업이 입력되기 전까지 **절대 장면 진행 불가**
  - 플레이어가 다른 행동 요청해도 무시하고 캐릭터 정보 입력 강제
  - 필수 항목: 이름(반드시), 직업(반드시). 나머지는 선택

- `c_llm_extract` (상태 추출 LLM) 프롬프트 [플레이어 정보 입력 감지] 수정
  - "실제로 답변한 내용" → **"새로 반영된 것이 보이면, 프리셋 선택이든 직접 입력이든"**
  - 프리셋 번호 선택("1번")도 player_patch로 인식하도록 확장
  - 서사에 플레이어 정보가 처음 나타나면 무조건 추출

---

## v62 (2026-07-21)

**사용자 지시**: /플레이 재진입 시 직전 턴 내용 반복 출력

**주요 변경**:
- `c_llm` (플레이 LLM) 프롬프트 [모드 진입 안내] 수정
  - `/플레이` 단독 입력(재진입) 시: 새 장면 생성 금지, 직전 턴 내용 90% 이상 복원
  - `recent_memories` 마지막 항목의 content를 읽어 서사/마인드모니터/상황판/선택지 전체 재출력
  - 수치나 선택지 내용 변경 금지, 몇 문장만 자연스럽게 다듬기만 허용
  - "새로 시작할까요" 등 되묻기 금지
  - 출력 순서: 게임제목+턴수(한 줄) → 서사 → 마인드모니터 → 상황판 → 선택지
...
- 플레이어 상황판: 상식개변 적용 현황 필수 출력

## v52 (2026-07-20)
- 플레이어 프리셋 5종 삭제 → 직접 입력 유도
- 마인드모니터 [1.표면의식]/[2.잠재의식] 대화체 전환
- 플레이어 심리 대화체 추가
- player 필드: occupation → job (DB 마이그레이션 완료)
- 성기 길이 필수 문구 제거
- reset_game_progress RPC 수정 (job 필드 초기화)
